<template>
  <div class="bg-white rounded-xl shadow-sm overflow-hidden">
    ...
    <div class="relative h-[600px] overflow-hidden">
      <div v-if="loading"
        class="absolute inset-0 bg-white/80 flex flex-col items-center justify-center z-10 text-slate-500">
      ...
      <div v-show="transactions.length > 0" class="h-full overflow-y-auto overflow-x-hidden" ref="scrollContainer">
        <div class="relative w-full" :style="{ height: virtualHeight + 'px' }">
          <div class="absolute top-0 left-0 right-0 px-6 py-3" v-for="item in visibleTransactions" :key="item.data.id"
                ...
                <div class="flex items-center justify-between gap-4 flex-col md:flex-row md:items-start">
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-3 mb-2">
                      <h3
                        class="m-0 text-base font-semibold text-slate-800 whitespace-nowrap overflow-hidden text-ellipsis">
                        {{ item.data.description }}</h3>
                      <ds-badge :variant="item.data.type === 'income' ? 'success' : 'danger'" size="sm">
                  ...
                  <div
                    class="flex flex-row items-center justify-between gap-2 w-full md:w-auto md:flex-row md:justify-between md:items-center">
                    <p
                      :class="['m-0 text-xl font-bold whitespace-nowrap md:text-lg', item.data.type === 'income' ? 'text-green-700' : 'text-red-700']">
                      {{ formatCurrency(item.data.value, item.data.type) }}
                    </p>
                    <ds-button variant="danger" size="sm" @click="handleDelete(item.data)">
